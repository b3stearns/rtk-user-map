...
const UserMap = () => {
  ...
  const [timeFilter, setTimeFilter] = React.useState('1d');
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      legend.onAdd = function () {
        ...
        try {
          const div = L.DomUtil.create('div', 'leaflet-control-legend bg-white p-2 border border-gray-300 rounded shadow-md');
          div.innerHTML = `
            <div class="legend-item flex items-center mb-2">
              <div class="w-3 h-3 bg-blue-700 rounded-full mr-2"></div>
              <span class="text-sm">Base Station</span>
            </div>
            <div class="legend-item flex items-center mb-2">
              <img src="/JohnDeereDevice.png" alt="John Deere" class="w-6 h-6 mr-2" onerror="this.style.display='none'; console.error('Failed to load John Deere icon');">
              <span class="text-sm">John Deere Device</span>
            </div>
            <div class="legend-item flex items-center mb-2">
              <img src="/iosdevice.png" alt="iPhone" class="w-6 h-6 mr-2" onerror="this.style.display='none'; console.error('Failed to load iPhone icon');">
              <span class="text-sm">iPhone Device</span>
            </div>
            <div class="legend-item flex items-center mb-2">
              <img src="/default.png" alt="Unknown" class="w-6 h-6 mr-2" onerror="this.style.display='none'; console.error('Failed to load Unknown icon');">
              <span class="text-sm">Unknown Hardware</span>
            </div>
          `;
          return div;
        } catch (error) {
      ...
      logoControl.onAdd = function () {
        ...
        try {
          const div = L.DomUtil.create('div', 'leaflet-control-logo bg-transparent border-none shadow-none');
          div.innerHTML = `
            <img src="/GeodnetLogo.png" alt="Geodnet Logo" class="w-[200px] h-auto" onerror="this.style.display='none'; console.error('Failed to load GeodnetLogo.png');">
          `;
          return div;
        } catch (error) {
